...
検証日: 2026-02-07  
検証シナリオ:
- AI x2 / casual / 10分 / seed 1001-1010（10試行）
- AI x5 / normal / 10分 / seed 2001-2010（10試行）
- AI x5 / casual / 10分 / seed 4001-4005（5試行）
- AI x80 / normal / 10分 / seed 3001-3002（2試行）

集計サマリ（抜粋）:
- AI x2 casual: all_down率 30%, timeout率 70%, avgMaxCapture 7.5%
- AI x5 casual: 勝利 0/5, timeout率 100%, avgMaxCapture 30%
- AI x5 normal: timeout率 100%, avgMaxCapture 22.5%
- AI x80 normal: timeout率 100%, avgMaxCapture 33.3%
- anomalies 検知率: 全シナリオ 0%

- [x] `NG` 5人でクリア可能か（カジュアル難易度）: 勝利確認できず。改善Issue [#42](https://github.com/mokemokechicken/mmo-packman/issues/42)
- [x] `保留` 100人でも終盤に緊張感があるか（ノーマル難易度）: 今回はAI x80（2試行）の近似検証のみで、100人実測が未実施。追試Issue [#43](https://github.com/mokemokechicken/mmo-packman/issues/43)
- [x] `保留` 維持コストカーブが急すぎて90%から先に進めない状況にならないか: 90%帯の到達データ不足
- [x] `NG` 少人数でボスゴースト撃破が現実的か（覚醒ストック3回の蓄積で対応可能か）: boss関連イベントがほぼ発生せず。改善Issue [#44](https://github.com/mokemokechicken/mmo-packman/issues/44)
- [x] `保留` フルーツの「アップル（遠隔復活）」が少人数プレイで必須すぎないか: 現行出力では果実種別寄与を定量化できない
- [x] `保留` ゲーム時間が目標範囲（15〜30分）に収まるか: 今回は10分固定シナリオ中心で未判定
- [x] `保留` 覚醒ゲージの必要ドット数（50個）が多すぎ / 少なすぎないか: A/B比較データ不足
- [x] `保留` パワーエサの再出現間隔（90秒）が適切か: A/B比較データ不足
- [x] `保留` AI代行プレイヤーが強すぎて人間不要にならないか: AI-only結果のみで、人間混在比較（human+AI / human-only）が未実施
- [x] `保留` AI代行プレイヤーが弱すぎて離脱発生時にゲーム崩壊しないか: 離脱注入シナリオ未実施（現状はAI x2 casualの間接指標のみ）。追試/改善Issue [#45](https://github.com/mokemokechicken/mmo-packman/issues/45)
- [x] `保留` 戦場の霧が序盤の探索を面白くしているか、ストレスになっていないか: 主観評価のプレイテストが未実施
- [x] `保留` 自動復活のストック喪失ペナルティが重すぎないか（特に少人数で救出が間に合いにくい場合）: 追加指標（復帰後の再崩壊率）が必要
